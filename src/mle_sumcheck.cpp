...
void sProver::initialize() {
    ...
    for (uint64_t mask = 0; mask < tsize; ++mask) {
        keepTable[mask] = g.get_term(mask);
    }

    for (uint64_t i = 0; i < g.num_vars; ++i) {
        for (uint64_t mask = 0; mask < tsize; ++mask) {
            if ((mask & (1ull << i)) == 0) {
    ...
    for(int round = 1;round <= nrnd; ++round){
        ...
        if(round == 1){
            ...
            if(round == nrnd){
                ...
                // fr: f(r1, r2, ..., rl)
                // Goldilocks2::Element f_r = query_oracle(oracle, challenges);
                Goldilocks2::Element f_r = oracle.evaluate(challenges);
